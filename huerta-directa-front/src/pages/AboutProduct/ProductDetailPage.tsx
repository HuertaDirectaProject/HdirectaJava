...
import { useProductDetail, type Comment } from "../../hooks/Productos/useProductDetail";

// ─── Sub-componente: sección de reseñas ──────────────────────────────────────
...
const ReviewSection: React.FC<ReviewSectionProps> = ({
  comments, newRating, setNewRating, newComment,
...
export const ProductDetailPage = () => {
  const {
    product, relatedProducts, comments, loading, submittingReview, currentUser,
    allImages, mainImageUrl, selectedImage, setSelectedImage,
    quantity, setQuantity, handleAddToCart,
    isFavorite, isFavoriteLoading, handleFavoriteToggle,
    activeTab, setActiveTab, newRating, setNewRating,
    newComment, setNewComment, handleSubmitReview,
    hasDiscount, discountedPrice, rating, reviewCount,
    navigate,
  } = useProductDetail();
  ...
  const handleContactSeller = () => {
    ...
    navigate(
      `/dashboard/mensajes?userId=${product.userId}&userName=${encodeURIComponent(product.userName ?? "Vendedor")}`
    );
  };
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {relatedProducts.map((p) => (
                <ProductCard key={p.idProduct}
                  product={{
                    id: p.idProduct, name: p.nameProduct, price: p.price,
                    stock: p.stock, image: `${API_URL}/uploads/productos/${p.imageProduct}`,
                    category: p.category, discountOffer: p.discountOffer, images: p.images,
                  }} />
              ))}
            </div>
          </div>
